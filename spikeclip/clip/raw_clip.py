...
class DirectResNetTransformer(nn.Module):
    def __init__(self,
                 frame_feature_dim: int,    # 帧特征维度
                 ):   
        super().__init__()
        
        # 使用标准ResNet18（输入通道设为原始视频通道数）
        self.resnet = torchvision.models.resnet18(pretrained=False)
        
        # 修改第一层卷积（假设原始视频每帧的通道数为C）
        original_channels = 10  # 根据实际情况修改为视频的通道数
        self.resnet.conv1 = nn.Conv2d(
            in_channels=original_channels,  # 原始输入通道
            out_channels=64,
            kernel_size=7,
            stride=2,
            padding=3,
        ...
        self.resnet.fc = nn.Linear(
            ...
            out_features=frame_feature_dim
        )
    ...
    def forward(self, video_frames: torch.Tensor) -> torch.Tensor:
        ...
        # 通过ResNet提取帧特征 [B*T, D]
        frame_features = self.resnet(frames_flat)
        
        # 恢复时间维度 [B, T, D]
        temporal_features = frame_features.view(B, T, -1)
        
        # 使用Transformer处理时序特征 [T, B, D]
        temporal_features = temporal_features.permute(1, 0, 2)
        
        # 全局平均池化 [B, D]
        global_feature = temporal_features.mean(dim=0)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.visual = DirectResNetTransformer(
